...
<div class="-mt-4 text-start *:leading-normal!">
  <GdgLogo class="w-16 h-auto mb-4" />
  <h1 class="font-500">
...
<h2 class="font-500 mt-4">以前都可以，SSR 為什麼不可以</h2>

---
clicks: 1
---

## 前幾天遇到一個有趣的 hydration 問題 {.font-500}
...
<img
  src="/hydration-completed but-contains mismatches.png"
  class="w-full h-auto rounded-t-xl transition-transform duration-500"
  :class="{ 'translate-y-[80%]': $clicks < 1 }"
  onload="this.style.visibility = 'visible';"
  style="filter:contrast(1.15);visibility:hidden;"
/>
...
## Server 生成的 HTML，瀏覽器不一定照做 {.font-500}
...
<div>
  <span class="px-1.5 text-xs text-green bg-green/10 rounded">原始碼</span>
</div>
...
<div>
  <span class="px-1.5 text-xs text-blue bg-blue/10 rounded">Server 輸出的 HTML</span>
</div>
...
<div>
  <span class="px-1.5 text-xs text-amber bg-amber/10 rounded">瀏覽器解析後實際的 DOM</span>
</div>
...
<div class="grid grid-cols-2 gap-x-6 gap-y-1 mt-4 items-start [&_pre]:!py-2">

<div class="text-center text-sm font-500 opacity-50">我們寫的 HTML</div>
<div class="text-center text-sm font-500 opacity-50">瀏覽器實際生成</div>
...
<hr class="col-span-2 border-white/20 my-2" />
...
<h2 class="font-500 mt-4 !leading-relaxed">
  瀏覽器會很聰明地默默修正錯誤的 HTML，<br />但 SSR 字串只會老實的顯示開發人員的設計
</h2>
