<template>
  <svg width="100%" viewBox="0 50 680 600" xmlns="http://www.w3.org/2000/svg">
    <defs>
      <marker id="ci-async-arrow" viewBox="0 0 10 10" refX="8" refY="5" markerWidth="6" markerHeight="6" orient="auto-start-reverse">
    ...
    <line v-for="(y, i) in arrowYs" :key="y" class="transition-opacity duration-300" :style="{ opacity: $clicks >= base + i + 1 ? 1 : 0 }"
      ...
      stroke="#73726C" stroke-width="1.5" marker-end="url(#ci-async-arrow)" />
  </svg>
</template>
...
type ColorKey = 'parent' | 'sibling' | 'action' | 'danger'
...
const colors: Record<ColorKey, { bg: string; stroke: string; title: string; sub: string }> = {
  ...
  sibling: { bg: '#E1F5EE', stroke: '#0F6E56', title: '#085041', sub: '#0F6E56' },
  action: { bg: '#F1EFE8', stroke: '#5F5E5A', title: '#444441', sub: '#5F5E5A' },
  danger: { bg: '#FCE8E8', stroke: '#B42318', title: '#7A1A11', sub: '#B42318' },
}
...
const rows: Row[] = [
  ...
  {
    y: 146,
    step: { color: 'sibling', title: '2. AsyncSibling setup() 開始' },
    action: { title: 'setCurrentInstance(AsyncSibling)' },
    current: { color: 'sibling', label: 'AsyncSibling' },
  },
  {
    y: 232,
    step: { color: 'sibling', title: '3. 遇到 await，setup() 暫停' },
    action: { title: 'unsetCurrentInstance()' },
    current: { color: 'parent', label: 'Parent' },
  },
  {
    y: 318,
    step: { color: 'sibling', title: '4. Sibling setup() 開始' },
    action: { title: 'setCurrentInstance(Sibling)' },
    current: { color: 'sibling', label: 'Sibling' },
  },
  {
    y: 404,
    step: { color: 'sibling', title: '5. Sibling setup() 結束' },
    action: { title: 'unsetCurrentInstance()' },
    current: { color: 'parent', label: 'Parent' },
  },
  {
  ...
  {
    y: 576,
    step: { color: 'sibling', title: '7. AsyncSibling await 完成' },
    action: { title: '繼續執行 setup' },
    current: { color: 'danger', label: 'null' },
  },
]
