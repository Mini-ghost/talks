...
type ColorKey = 'parent' | 'child' | 'action'
...
const colors: Record<ColorKey, { bg: string; stroke: string; title: string; sub: string }> = {
  ...
  child: { bg: '#E1F5EE', stroke: '#0F6E56', title: '#085041', sub: '#0F6E56' },
...
const rows: Row[] = [
  ...
  {
    y: 146,
    step: { color: 'parent', title: '2. onMounted()' },
    action: { title: '讀取 currentInstance' },
    current: { color: 'parent', label: 'Parent' },
  },
  {
    y: 232,
    step: { color: 'child', title: '3. Child setup() 開始' },
    action: { title: 'setCurrentInstance(Child)' },
    current: { color: 'child', label: 'Child' },
  },
  {
    y: 318,
    step: { color: 'child', title: '4. onMounted()' },
    action: { title: '讀取 currentInstance' },
    current: { color: 'child', label: 'Child' },
  },
  {
    y: 404,
    step: { color: 'child', title: '5. Child setup() 結束' },
